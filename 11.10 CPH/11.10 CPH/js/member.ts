...
import { Duty, get_duties, sign_in, ask_for_absent, ask_for_substitute, Substitute, accept_substitute, refuse_substitute, get_substitutes, DutyRecord, get_records } from "./post_api";

window.onload = function() {
	alert("loading")
	// 检查token并决定是否跳转
	for (let duty of get_duties()) {
		(<HTMLElement>document.getElementById("week-table")).appendChild(dutyToHTML(duty));
	}
	for (let request of get_substitutes()) {
		(<HTMLElement>document.getElementById("requests-table")).appendChild(substituteToHTML(request));
	}
	for (let record of get_records()) {
		(<HTMLElement>document.getElementById("records-table")).appendChild(recordToHTML(record));
	}
}

function dutyToHTML(duty: Duty): HTMLFormElement {
	...
	formElem.sign_in.onclick = function() {
		if (sign_in()) alert("签到成功！");
		else alert("签到失败，请稍后重试"); // 对返回值分类？比如已迟到比如不当值比如提交失败
	};
	formElem.absent.onclick = function() {
		if (ask_for_absent(duty)) alert("请求已发送");
		else alert("请求发送失败，请稍后重试");
	};
...
function substituteToHTML(request: Substitute): HTMLFormElement {
	var formElem = document.createElement("form");
	formElem.className = "form-inline";
	formElem.innerHTML = `
		<div class="form-group" style="margin-right:10px;">
...
function recordToHTML(record: DutyRecord): HTMLTableRowElement {
